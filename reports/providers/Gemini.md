...
| `gemini-2.0-flash-thinking` | text | ✅ `ok` | 1.65s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking-with-apps` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 20.07s | contains expected token 'PONG' |
| `gemini-3.1-flash-lite` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-3.5-flash` | text | ✅ `ok` | 21.79s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite-thinking` | text | ✅ `ok` | 43.17s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking` | text | ✅ `ok` | 1.79s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking-lite` | text | ✅ `ok` | 20.12s | contains expected token 'PONG' |
| `gemini-3.6-flash-thinking` | text | ✅ `ok` | 1.81s | contains expected token 'PONG' |
...
| `gemini-3.6-flash` | image | ❌ `exception` | 1.13s | NoMediaResponseError: No media response from Gemini |
