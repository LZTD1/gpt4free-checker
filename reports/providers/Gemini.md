...
- **Models:** 13
- **Working tests:** 4 / 13
- **Avg response time:** 21.75s

## Per-model results

| Model | Capability | Status | Time | Notes |
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 21.98s | contains expected token 'PONG' |
| `gemini-2.0-flash` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 20.98s | contains expected token 'PONG' |
...
| `gemini-3.1-flash-lite` | text | ❌ `api_error` | 0.01s | MissingAuthError: Gemini session is unauthenticated; model 'gemini-flash-lite' would fall back to Flash |
...
| `gemini-3.5-flash` | image | ❌ `exception` | 22.03s | NoMediaResponseError: No media response from Gemini |
| `gemini-3.5-flash-thinking` | text | ❌ `api_error` | 0.01s | MissingAuthError: Gemini session is unauthenticated; model 'gemini-3.5-flash-thinking' would fall back to Flash |
...
| `gemini-flash-lite` | text | ❌ `api_error` | 0.01s | MissingAuthError: Gemini session is unauthenticated; model 'gemini-flash-lite' would fall back to Flash |
