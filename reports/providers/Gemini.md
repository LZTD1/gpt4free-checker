...
- **Models:** 17
- **Working tests:** 14 / 17
- **Avg response time:** 18.54s

## Per-model results

| Model | Capability | Status | Time | Notes |
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 1.79s | contains expected token 'PONG' |
| `gemini-2.0-flash` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking` | text | ✅ `ok` | 2.01s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking-with-apps` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 21.94s | contains expected token 'PONG' |
| `gemini-3.1-flash-lite` | text | ✅ `ok` | 20.25s | contains expected token 'PONG' |
...
| `gemini-3.5-flash-lite-thinking` | text | ✅ `ok` | 21.54s | contains expected token 'PONG' |
...
| `gemini-3.6-flash` | image | ❌ `exception` | 20.50s | NoMediaResponseError: No media response from Gemini |
