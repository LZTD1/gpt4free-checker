...
- **Models:** 17
- **Working tests:** 14 / 17
- **Avg response time:** 17.06s

## Per-model results

| Model | Capability | Status | Time | Notes |
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 1.93s | contains expected token 'PONG' |
| `gemini-2.0-flash` | text | ✅ `ok` | 21.79s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking-with-apps` | text | ✅ `ok` | 20.08s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 1.87s | contains expected token 'PONG' |
| `gemini-3.1-flash-lite` | text | ✅ `ok` | 22.02s | contains expected token 'PONG' |
...
| `gemini-3.5-flash-thinking` | text | ✅ `ok` | 20.43s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking-lite` | text | ✅ `ok` | 20.44s | contains expected token 'PONG' |
| `gemini-3.6-flash-thinking` | text | ✅ `ok` | 20.16s | contains expected token 'PONG' |
...
| `gemini-3.6-flash` | image | ❌ `exception` | 20.29s | NoMediaResponseError: No media response from Gemini |
