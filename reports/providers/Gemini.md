...
- **Models:** 13
- **Working tests:** 4 / 13
- **Avg response time:** 21.85s

## Per-model results

| Model | Capability | Status | Time | Notes |
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 21.50s | contains expected token 'PONG' |
| `gemini-2.0-flash` | text | ✅ `ok` | 22.00s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 22.08s | contains expected token 'PONG' |
...
| `gemini-3.5-flash` | image | ❌ `exception` | 43.78s | NoMediaResponseError: No media response from Gemini |
