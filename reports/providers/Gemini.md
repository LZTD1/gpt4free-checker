...
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 20.15s | contains expected token 'PONG' |
| `gemini-2.0-flash` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 21.83s | contains expected token 'PONG' |
...
| `gemini-3.5-flash` | image | ❌ `exception` | 21.02s | NoMediaResponseError: No media response from Gemini |
