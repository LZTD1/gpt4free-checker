...
- **Models:** 17
- **Working tests:** 14 / 17
- **Avg response time:** 21.81s

## Per-model results

| Model | Capability | Status | Time | Notes |
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 20.69s | contains expected token 'PONG' |
| `gemini-2.0-flash` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking` | text | ✅ `ok` | 43.46s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking-with-apps` | text | ✅ `ok` | 22.00s | contains expected token 'PONG' |
...
| `gemini-3.5-flash` | text | ✅ `ok` | 44.22s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite` | text | ✅ `ok` | 20.52s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite-thinking` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking` | text | ✅ `ok` | 1.91s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking-lite` | text | ✅ `ok` | 22.20s | contains expected token 'PONG' |
| `gemini-3.6-flash-thinking` | text | ✅ `ok` | 22.04s | contains expected token 'PONG' |
| `gemini-auto` | text | ✅ `ok` | 1.97s | contains expected token 'PONG' |
| `gemini-flash-lite` | text | ✅ `ok` | 21.05s | contains expected token 'PONG' |
