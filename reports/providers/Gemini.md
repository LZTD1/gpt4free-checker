...
| `gemini-2.0-flash` | text | ✅ `ok` | 20.19s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-auto` | text | ✅ `ok` | 2.05s | contains expected token 'PONG' |
...
| `gemini-3.5-flash` | image | ❌ `exception` | 1.10s | NoMediaResponseError: No media response from Gemini |
