...
| `gemini-2.0-flash-thinking` | text | ✅ `ok` | 21.60s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking-with-apps` | text | ✅ `ok` | 1.70s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 43.35s | contains expected token 'PONG' |
| `gemini-3.1-flash-lite` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-3.5-flash` | text | ✅ `ok` | 20.43s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite-thinking` | text | ✅ `ok` | 1.95s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking-lite` | text | ✅ `ok` | 20.53s | contains expected token 'PONG' |
| `gemini-3.6-flash-thinking` | text | ✅ `ok` | 42.70s | contains expected token 'PONG' |
| `gemini-auto` | text | ✅ `ok` | 22.02s | contains expected token 'PONG' |
| `gemini-flash-lite` | text | ✅ `ok` | 1.82s | contains expected token 'PONG' |
...
| `gemini-3.6-flash` | image | ❌ `exception` | 20.23s | NoMediaResponseError: No media response from Gemini |
