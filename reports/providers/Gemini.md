...
| `gemini-2.0-flash` | text | ✅ `ok` | 22.26s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 21.82s | contains expected token 'PONG' |
...
| `gemini-3.5-flash` | image | ❌ `exception` | 21.03s | NoMediaResponseError: No media response from Gemini |
