...
| `gemini-2.0-flash` | text | ✅ `ok` | 21.87s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 20.63s | contains expected token 'PONG' |
...
| `gemini-3.5-flash` | image | ❌ `exception` | 21.06s | NoMediaResponseError: No media response from Gemini |
