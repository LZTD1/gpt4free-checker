...
- **Models:** 17
- **Working tests:** 14 / 17
- **Avg response time:** 22.97s

## Per-model results

| Model | Capability | Status | Time | Notes |
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 41.68s | contains expected token 'PONG' |
| `gemini-2.0-flash` | text | ✅ `ok` | 1.87s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking` | text | ✅ `ok` | 21.88s | contains expected token 'PONG' |
| `gemini-2.0-flash-thinking-with-apps` | text | ✅ `ok` | 22.02s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 20.12s | contains expected token 'PONG' |
| `gemini-3.1-flash-lite` | text | ✅ `ok` | 22.10s | contains expected token 'PONG' |
| `gemini-3.5-flash` | text | ✅ `ok` | 43.37s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite` | text | ✅ `ok` | 20.14s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite-thinking` | text | ✅ `ok` | 20.27s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking` | text | ✅ `ok` | 21.87s | contains expected token 'PONG' |
