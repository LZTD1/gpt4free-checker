...
- **Models:** 17
- **Working tests:** 14 / 17
- **Avg response time:** 19.07s

## Per-model results

| Model | Capability | Status | Time | Notes |
| --- | --- | :---: | ---: | --- |
| `gemini-2.0` | text | ✅ `ok` | 20.34s | contains expected token 'PONG' |
...
| `gemini-2.0-flash-thinking-with-apps` | text | ✅ `ok` | 2.42s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-3.1-flash-lite` | text | ✅ `ok` | 25.86s | contains expected token 'PONG' |
| `gemini-3.5-flash` | text | ✅ `ok` | 20.26s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-3.5-flash-lite-thinking` | text | ✅ `ok` | 22.10s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking` | text | ✅ `ok` | 22.03s | contains expected token 'PONG' |
| `gemini-3.5-flash-thinking-lite` | text | ✅ `ok` | 1.79s | contains expected token 'PONG' |
| `gemini-3.6-flash-thinking` | text | ✅ `ok` | 21.03s | contains expected token 'PONG' |
| `gemini-auto` | text | ✅ `ok` | 22.01s | contains expected token 'PONG' |
...
| `gemini-3.6-flash` | image | ❌ `exception` | 20.33s | NoMediaResponseError: No media response from Gemini |
