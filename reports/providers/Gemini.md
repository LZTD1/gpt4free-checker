...
| `gemini-2.0-flash` | text | ✅ `ok` | 20.33s | contains expected token 'PONG' |
| `gemini-2.5-flash` | text | ✅ `ok` | 20.77s | contains expected token 'PONG' |
| `gemini-auto` | text | ✅ `ok` | 20.36s | contains expected token 'PONG' |
...
| `gemini-3.5-flash` | image | ❌ `exception` | 1.00s | NoMediaResponseError: No media response from Gemini |
