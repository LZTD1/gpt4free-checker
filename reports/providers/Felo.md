...
| --- | --- | :---: | ---: | --- |
| `felo-chat` | text | ✅ `ok` | 1.87s | contains expected token 'PONG' |
| `felo-document` | text | ✅ `ok` | 8.34s | contains expected token 'PONG' |
| `felo-scholar` | text | ❌ `rate_limited` | 0.22s | RateLimitError: Response 429: {'detail': 'Rate limit exceeded.'} |
| `felo-search` | text | ❌ `exception` | 5.02s | TypeError: sequence item 2: expected str instance, Sources found |
